...
const Chat = ({ username, onLogout, authManager }) => {
  ...
  const [profileAvatars, setProfileAvatars] = useState({}); // Кэш аватарок
  const [myProfile, setMyProfile] = useState(null); // Свой профиль
  const messagesEndRef = useRef(null);
  const searchTimeoutRef = useRef(null);

  // Загрузить свой профиль при монтировании
  useEffect(() => {
    loadMyProfile();
  }, [username]);

  // Загрузить аватарки для всех контактов
  useEffect(() => {
    contacts.forEach((contact) => {
      if (!profileAvatars[contact.username]) {
        loadProfileAvatar(contact.username);
      }
    });
  }, [contacts]);
  ...
  const loadProfileAvatar = async (username) => {
    try {
      const response = await fetch(
        `${authManager.serverUrl}/profile/${username}`
      );
  ...
  const handleProfileEditClose = () => {
    ...
    loadMyProfile(); // Перезагрузить свой профиль после редактирования
  };
      ...
      <div className="chat-layout">
        ...
        <div className={`burger-panel ${menuOpen ? "open" : ""}`}>
          <div className="user-profile">
            {/* Используем тернарный оператор для аватарки */}
            {myProfile?.avatar ? (
              <div
